...
import com.openrealm.data.entity.auth.AccountEntity.AccountEntityBuilder;
import com.openrealm.data.entity.auth.AccountPropertyEntity;
import com.openrealm.data.entity.auth.AccountProvisionEntity;
import com.openrealm.data.entity.auth.AccountTokenEntity;
import com.openrealm.data.repository.auth.AccountAccessRepository;
import com.openrealm.data.repository.auth.AccountAuthRepository;
import com.openrealm.data.repository.auth.AccountPropertyRepository;
import com.openrealm.data.repository.auth.AccountProvisionRepository;
...
import com.openrealm.data.util.SHAHash;
import com.openrealm.data.util.SHAValidate;
...
public class AccountService {
        ...
        try {
            ...
            if (this.accountRepo.count() == 0L) {
                ...
                for (AccountDto account : accounts) {
                    final AccountEntity createdAccount = this.registerStandardAccount(account);
    ...
    public AccountEntity registerStandardAccount(AccountDto account) throws Exception {
        try {
    ...
    public void changePassword(String accountGuid, String currentPassword, String newPassword) throws Exception {
        ...
        if (!com.openrealm.data.util.SHAValidate.validatePassword(currentPassword, auth.getPassword())) {
            throw new Exception("Current password is incorrect");
        }
        String newHash = com.openrealm.data.util.SHAHash.generateStrongPasswordHash(newPassword);
    ...
    public AccountDto getAccountByToken(String apiToken) throws Exception {
        final Exception errorMsg = new Exception("Invalid OpenRealm API Token");
